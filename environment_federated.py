...
#Test label prediction function    
    ...
    def run_experiment(self, attack_type = 'no_attack', malicious_behavior_rate = 0,
        ...
        simulation_model2 = copy.deepcopy(self.global_model) 
        print('\n===>Simulation started...')
        lfd = LFD(self.num_classes)
        fg = FoolsGold(self.num_peers)
        # copy weights
        global_weights = simulation_model.state_dict()
        local_weights_avg = simulation_model.state_dict()
        last10_updates = []
        test_losses = []
        global_accuracies = []
        ...
        for epoch in tqdm_notebook(range(start_round, self.global_rounds)):
            ...
            local_weights, local_grads, local_models, local_losses, performed_attacks, local_weights_ours, local_weights_modify = [], [], [], [], [], [], []  
            peers_types = []
            i = 1        
            attacks = 0
            ...
            if rule == 'fedavg':
                ...
                cpu_runtimes.append(time.time() - cur_time)
            elif rule == 'ppfl_defender':
                cur_time = time.time()
                scores = lfd.ppfl_cossim(copy.deepcopy(simulation_model), local_grads)
                global_weights = average_weights(local_weights, scores)
                end_time = time.time() - cur_time
                print("aggregate time:")
                print(end_time)
                cpu_runtimes.append(time.time() - cur_time)
            elif rule == 'tmean':
                    cur_time = time.time()
